    ...
    <div class="row">
      ...
      <p></p>
       <div class="nav flex-column nav-pills" aria-orientation="vertical">
          ...
          <a class="nav-link padding15 {{ Nav::isRoute('user.order') }} {{ Nav::isRoute('user.view.order') }}" href="{{ url('/order') }}"> <i class="fa fa-dot-circle-o" aria-hidden="true"></i> {{ __('staticwords.MyOrders') }}</a>
...
<div class="bg-white2 view-full-order-page">
  ...
  <a title="{{ __('Go back') }}" href="{{ url('/order') }}" class="btn btn-sm btn-default"><i class="fa fa-reply" aria-hidden="true"></i>
  ...
  @if(isset($order))
    @foreach($order->invoices as $sorder)
      @if($sorder->status == 'delivered' || $sorder->status == 'cancel_request' || $sorder->status =='return_request' || $sorder->status == 'returned' || $sorder->status == 'refunded' || $sorder->status == 'ret_ref')
        @php
  ...
  @if(in_array(1, $cancel_valid) && $tstatus != 1 && empty($orderlog) && empty($checkOrderCancel))
    <a title="Cancel FULL Order?" data-toggle="modal" data-target="#cancelFULLOrder" class="pull-right btn btn-md btn-danger text-white">
        {{ __('Cancel Order') }}
    </a>
  @endif
...
@if(!isset($checkOrderCancel) || !isset($orderlog))
  <!-- Modal -->
<div data-backdrop="static" data-keyboard="false" class="z-index99 modal fade" id="cancelFULLOrder" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h5 class="modal-title" id="myModalLabel">{{ __('staticwords.CancelOrder') }}: #{{ $inv_cus->order_prefix.$order->order_id }}</h5>
      </div>
    ...
    <form method="POST" action="{{ route('full.order.cancel',$secureorderID) }}">
      @csrf
        ...
        <div class="form-group">
         <label class="font-weight-normal" for="">{{ __('staticwords.ChooseReason') }} <span class="required">*</span></label>
          <select class="form-control" required="" name="comment" id="">
              ...
              <option value="Product is not match to my expectations">{{ __('Product is not match to my expectations') }}</option>
        ...
        @if($order->payment_method !='COD' && $order->payment_method !='BankTransfer')
           <div class="form-group">
              ...
              <label class="font-weight-normal"><input required class="source_check" type="radio" value="orignal" name="source" />{{ __('staticwords.OrignalSource') }} [{{ $order->payment_method }}] </label>&nbsp;&nbsp;
              ...
              @else
                ...
                <label class="font-weight-normal"><input type="radio" disabled="" /> {{ __('staticwords.InBank') }} <i class="fa fa-question-circle" data-toggle="tooltip" data-placement="right" title="Add a bank account in My Bank Account" aria-hidden="true"></i></label>
        ...
        <button @if(env('DEMO_LOCK') == 0) type="submit" @else disabled="disabled" title="This action is disabled in demo !" @endif class="btn btn-md btn-info">
...
@endif
            ...
            <table class="table table-striped table-striped-one">
              ...
              <p class="font-weight-normal"><i class="fa fa-map-marker" aria-hidden="true"></i> {{ strip_tags($address->address) }},</p>
              @php
              ...
              <p class="font-weight-normal margin-left8">{{ $address->pin_code }}</p>
                      ...
                      <td>
                  <p><b><i class="fa fa-user-circle"></i> {{ $order->billing_address['firstname'] }}, {{ $order->billing_address['mobile'] }}</b></p>
                  <p class="font-weight-normal"><i class="fa fa-map-marker" aria-hidden="true"></i> {{ strip_tags($order->billing_address['address']) }},</p>
                  @php
                  ...
                  @endphp
                  <p class="font-weight-normal margin-left8">{{ $ci }}, {{ $s }}, {{ $ci }}</p>
                  <p class="font-weight-normal margin-left8">@if(isset($order->billing_address['pincode'])) {{ $order->billing_address['pincode'] }} @endif</p>
                </td>
                    </tr>
                  </tbody>
            </table>
            ...
            @foreach($order->invoices as $o)
                ...
                <div class="panel-heading">
                  <div class="panel-title">
                    ...
                    @if($orivar->products->cancel_avl == '1')
                            ...
                            @endphp
                            <button @if(env('DEMO_LOCK') == 0) title="Cancel This Order?" data-toggle="modal" data-target="#proceedCanItem{{ $o->id }}" @else disabled="disabled" title="This action is disabled in demo !" @endif class="pull-right btn btn-sm btn-danger cancel-label">
                              {{ __('Cancel') }}
                            </button>
                            @endif
                          @else
                          ...
                          @if($orivar->products->return_avbl == '1')
                            @if($o->status == 'delivered')
                              ...
                              <!--Secure OrderItem-->
                                  ...
                                  @endphp
                              <!--END-->
                              <a href="{{ route('return.window',$secureInv) }}"><button class="m-l-8 pull-right btn btn-sm btn-danger">
                                {{ __('Return') }}
                              </button></a>
                              @endif
                            @endif
                          ...
                          @endif
                           ...
                           @if($o->status == 'delivered' || $o->status == 'return_request')
                            <a title="Click to View or print" href="{{ route('user.get.invoice',$o->id) }}" class="pull-right btn btn-sm btn-danger"><i class="fa fa-file-pdf-o" aria-hidden="true"></i> {{ __('Invoice') }}</a>
                          @endif
                ...
                <div class="panel-body">
                       ...
                       <span>Order Status for <b>{{$orivar->products->name}} <small>
                        (@foreach($orivar->main_attr_value as $key=> $orivars)
                          ...
                          @endforeach
                            ...
                            <a onclick="showOrder('{{ $o->id }}')" title="Close" class="pull-right"><i class="closef fa fa-window-close" aria-hidden="true"></i></a>
                  ...
                  <div id="OrderRow{{ $o->id }}" class="row full-order-main-block">
                    <div class="col-lg-1 col-md-2 col-sm-3 col-4">
                      <center><img class="pro-img2" src="{{url('variantimages/'.$orivar->variantimages['image2'])}}" alt=""/></center>
                    </div>
                      ...
                      <div>
                          ...
                          @php
                            ...
                            @if(isset($refundlog))
                              ...
                              @if($refundlog->status == 'initiated')
                                <small class="font-weight600">{{ __('Return Request Intiated with Ref. No:') }} [{{ $refundlog->txn_id }}]
                              ...
                              @if($refundlog->method_choosen == 'orignal')
                                ...
                                <small class="font-weight600">{{ __('Refund Amount') }} <i class="fa {{ $o->order->paid_in }}"></i>{{ $refundlog->amount }} {{ __('is') }} {{$refundlog->status}} {{ __('to your Requested payment source') }} {{ $refundlog->pay_mode }} {{ __('and will be reflected to your a/c in 1-2 working days.') }} <br> ({{ __('TXN ID:') }} {{ $refundlog->txn_id }})
                                </small>

                              @else

                                <small class="font-weight600">
                                   {{ __('Refund Amount') }} <i class="fa {{ $o->order->paid_in }}"></i>{{ $refundlog->amount }} is {{$refundlog->status}} {{ __('to your Requested bank a/c') }} <u>{{$refundlog->bank->bankname}} (XXXX{{ substr($refundlog->bank->acno, -4) }})</u> @if($refundlog->status !='refunded') {{ __('and will be reflected to your a/c in 1-2 working days.') }}@endif <br> (TXN ID: {{ $refundlog->txn_id }})
                                   .
                                   <br>
                                   @if($refundlog->txn_fee != '')
                                      {{ __('Transcation FEE Charge:') }} <i class="fa {{ $o->order->paid_in }}"></i>{{ $refundlog->txn_fee }}
                                   @endif
                                </small>
                        ...
                        @if(isset($log))
                          ...
                          @if($log->method_choosen == 'orignal')
                            ...
                            <small class="text-justify"><b>Refund Amount <u><i class="fa {{ $o->order->paid_in }}"></i>{{$log->amount}}</u> {{ __('is refunded to original source') }} ({{ $o->order->payment_method }}). {{ __("IF it don't than it will take 1-2 days to reflect in your account.") }} <br>({{ __("TXN ID:") }} {{ $log->transaction_id }})</b></small>
                            @elseif($log->method_choosen == 'bank' && $log->is_refunded == 'pending' )
                            <small><b>{{ __('Refund Amount') }} <u><i class="fa {{ $o->order->paid_in }}"></i>{{$log->amount}}</u> {{ __('is proceeded to your bank ac, amount will be reflected to your bank ac in 14 working days.') }} <br>
                            ...
                            <small><b>{{ __('Amount') }} <u><i class="fa {{ $o->order->paid_in }}"></i>{{$log->amount}}</u> {{ __('is refunded to your bank ac.') }} <br>
                              @if($log->txn_fee !='')
                          ...
                          @elseif(isset($orderlog))
                              ...
                              @if(in_array($o->id, $orderlog->inv_id))
                                ...
                                @endif  
                                 ...
                                 </u> {{ __('is refunded to original source') }} ({{ $o->order->payment_method }}). {{ __("IF it don't than it will take 1-2 days to reflect in your account.") }} <br>({{ __("TXN ID:") }} {{ $orderlog->txn_id }})</b></small>
                                ...
                                @endif  
                                  ...
                                  </u> {{ __("is proceeded to your bank ac, amount will be reflected to your bank ac in 14 working days.") }} <br>
                              ...
                              @if($orderlog->method_choosen == 'bank' && $orderlog->is_refunded == 'completed' )
                                ...
                                @if(in_array($o->id, $orderlog->inv_id))
                                  <small><b>{{ __('Amount') }} <u><i class="fa {{ $o->order->paid_in }}"></i> @if($o->order->discount !=0)
                            ...
                            @if($o->local_pick =='')
                             @if($o->status == 'pending' || $o->status == 'processed' || $o->status == 'shipped')
                             <button type="button"  id="btn_track{{ $o->id }}" onclick="trackorder('{{ $o->id }}')" class="btn btn-md btn-info">{{ __('staticwords.Track') }}</button>
                              @endif
                              @else
                                @if($o->status != 'delivered' && $o->status !='refunded' && $o->status !='ret_ref' && $o->status !='returned' && $o->status != 'canceled' && $o->status != 'return_request')
                                <div class="col-md-8 alert alert-info">
                                    <p><i class="fa fa-calendar-check-o" aria-hidden="true"></i>
                                      {{ __('staticwords.lpickupdatetext') }}
                                    </p>
                                      <p class="font-weight600">{{ $o->loc_deliv_date == '' ? "Yet to update" : date('d/m/Y',strtotime($o->loc_deliv_date)) }} • <a title="Click to see store address" class="know_more" data-toggle="modal" data-target="#localpickModal{{ $o->id }}">Know More...</a> </p>
              ...
              <!-- Modal -->
                          <div data-backdrop="static" data-keyboard="false" class="z-index99 modal fade" id="proceedCanItem{{ $o->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                            <div class="modal-dialog" role="document">
                              <div class="modal-content">
                                <div class="modal-header">
                                  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        ...
                        (@foreach($orivar->main_attr_value as $key=> $orivars)
                          ...
                          @endforeach
                                ...
                                <div class="modal-body">
                                  @if($orivar->products->cancel_avl == '1' && $o->status != 'canceled' && $o->status != 'refunded' && $o->status != 'Refund Pending' && $o->status != 'returned' && $o->status != 'delivered' && $o->status != 'return_request' && $o->status != 'ret_ref')
                                  <form action="{{ route('cancel.item',$secureid) }}" method="POST">
                                    @csrf
                                      ...
                                      <div class="form-group">
                                        ...
                                        @if(Auth::user()->banks->count()>0)
                                        <label class="font-weight-normal"><input onclick="showBank('{{ $o->id }}')" id="source_check_b{{ $o->id }}" required type="radio" value="bank" name="source" />
                                        {{ __('In Bank') }}</label>
                                        @else
                                      ...
                                      @else
                                        @if(Auth::user()->banks->count()>0)
                                        <label class="font-weight-normal"><input onclick="showBank('{{ $o->id }}')" id="source_check_b{{ $o->id }}" required type="radio" value="bank" name="source" />{{ __('staticwords.InBank') }}</label>
                                        @else
                                        <label class="font-weight-normal"><input disabled="disabled" type="radio"/> {{ __('staticwords.InBank') }} <i class="fa fa-question-circle" data-toggle="tooltip" data-placement="right" title="Add a bank account in My Bank Account" aria-hidden="true"></i></label>
                                        @endif
                                ...
                                <div class="modal-dialog" role="document">
                                  <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <p>{{ __('Pick your Ordered Item') }} <b>{{ $o->variant->products->name }} <small>(@foreach($orivar->main_attr_value as $key=> $orivars)
                          <?php $i++;?>
            ...
            @endforeach
                     ...
                     <div class="panel panel-default">
                       <div class="panel-footer">
                          ...
                          <p class="btmText"><b>{{ __('staticwords.Total') }}:</b> <i class="{{ $order->paid_in }}"></i>{{ round($order->order_total,2) }}</p>
                        
                         <p class="btmText"><b>{{ __('staticwords.HandlingCharge') }}:</b> <i class="{{ $order->paid_in }}"></i>{{ round($order->handlingcharge,2) }}</p>

                         <p class="btmText"><b>{{ __('staticwords.OrderTotal') }}:</b> <i class="{{ $order->paid_in }}"></i>{{ round(($order->order_total+$order->handlingcharge),2) }}</p>
