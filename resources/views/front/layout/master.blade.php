...
@php
  ...
  $seo = App\Seo::first();
   ...
   App\Jobs\CartPriceChange::dispatchNow();
...
<meta name="Description" content="{{$seo->metadata_des}}" />
<meta name="keyword" content="{{ $seo->metadata_key }}">
<meta name="robots" content="all">
<meta name="csrf-token" content="{{csrf_token()}}">
<!-- Theme Header Color -->
<meta name="theme-color" content="#157ED2">
<title>@yield('title') {{ $title}} - {{ $seo->metadata_des }} </title>
<!-- Google Fonts -->
...
<link rel="stylesheet" href="{{url('front/vendor/css/owl.carousel.css')}}">
<link rel="stylesheet" href="{{url('front/vendor/css/owl.transitions.css')}}">
<link rel="stylesheet" href="{{url('css/vendor/animate.min.css')}}">
<link rel="stylesheet" href="{{url('front/vendor/css/rateit.css')}}">
...
<link rel="stylesheet" href="{{ url('css/user-style.css') }}" compile>
<!-- Fontawesome icons -->
...
<script src="{{ url('js/front-head.js') }}"></script> 
<!-- Custom script head -->
@yield('head-script') 
<!-- Custom style head -->
@yield('stylesheet') 
    ...
    <div class="display-none payment-verify-preloader">
        <div class="payment">
            <div class="payment-message">
               <i class="fa fa-spinner fa-pulse fa-fw"></i> <span class="sr-only">{{ __('Loading') }}...</span> {{ __('staticwords.verifyPayment') }}
               <br>
            ...
            <div class="header-top-inner">
              <div class="cnt-account">
                ...
                <ul class="list-unstyled">
                  ...
                  @if(Auth::check())
                   ...
                   <li class="dropdown notifications-menu">
                    <a title="{{ __('staticwords.notification') }}" href="#" class="dropdown-toggle" data-toggle="dropdown">
                  ...
                  <ul id="dropdown" class="z-index99 dropdown-menu">
                    <li class="notiheadergrey header">@if(auth()->user()->unreadnotifications->where('n_type','!=','order_v')->count())
                     {{ __('staticwords.Youhave') }} <b>{{ auth()->user()->unreadnotifications->where('n_type','!=','order_v')->count() }}</b> {{ __('staticwords.notification') }} !                     
                     ...
                     @endif 
                      ...
                      <ul class="menu notification-menu">
                         ...
                         @foreach(auth()->user()->unreadNotifications as $notification)
                          ...
                          <li class="notiheaderlightgrey hey1" id="{{ $notification->id }}" onclick="markread('{{ $notification->id }}')">
                            <p></p>
                            <small class="padding5P float-right"><i class="fa fa-clock-o" aria-hidden="true"></i> {{ date('jS M y',strtotime($notification->created_at)) }}</small>                     
                            <a class="font-weight600 color111"                       
                            @if($notification->n_type == "order")
                ...
                @if(Auth::user()->role_id == "a")
                  <li class="first"><a target="_blank" title="GO to Admin Panel"  href="{{route('admin.main')}}" title="Admin">Admin</a></li>
                @elseif(Auth::user()->role_id == 'v')
                @if(isset(Auth::user()->store))
                  <li class="first"><a target="_blank" title="{{ __('staticwords.SellerDashboard') }}"  href="{{route('seller.dboard')}}" title="Admin">{{ __('staticwords.SellerDashboard') }}</a>
                  </li>
                @endif
                @endif
                  <li class="myaccount"><a href="{{url('profile')}}" title="My Account"><span>{{ __('staticwords.MyAccount') }}</span></a></li>
                  ...
                  @endphp
                   <li class="wishlist"><a href="{{url('wishlist')}}" title="Wishlist"><span>{{ __('staticwords.Wishlist') }}(<span id="wishcount">{{$wishcount}}</span>)</a></li>
                  @endif
                  @if(Auth::check())
                  <li class="login">
                  ...
                  @auth
                  <li class="check"><a data-toggle="modal" href="#feed" title="Feedback"><span>{{ __('staticwords.Feedback') }}</span></a></li>
      
                  <li><a href="{{ route('hdesk') }}" title="Help Desk & Support">{{ __('staticwords.hpd') }}</a></li>
        
                  <!-- Feedback Modal -->
                  <div data-backdrop="static" data-keyboard="false" class="z-index99 modal fade" id="feed" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                        ...
                        <div class="modal-body">
                          ...
                          <form action="{{ route('send.feedback') }}" method="POST">
                            ...
                            <div class="form-group">
                              <label class="font-weight-bold" for="">{{ __('staticwords.Name') }}: <span class="required">*</span></label>
                            <input required="" type="text" name="name" class="form-control" value="{{ $auth->name }}">
                            </div>                    
                            <div class="form-group">
                              <label class="font-weight-bold" for="">{{ __('staticwords.Email') }}: <span class="required">*</span></label>
                            <input required="" type="email" name="email" class="form-control" value="{{ $auth->email }}">
                            </div>
                            <div class="form-group">
                              <label class="font-weight-bold" for="">{{ __('staticwords.Message') }}: <span class="required">*</span></label>
                              <textarea required name="msg" placeholder="Tell us What You Like about us? or What should we do to more to improve our portal." cols="30" rows="10" class="form-control"></textarea>
                            </div>
                            <div class="rat">
                            <label class="font-weight-bold">&nbsp;{{ __('staticwords.RateUs') }}: <span class="required">*</span></label>
                    ...
                    @if($auto->currency_by_country == 1)
                      @if(!empty($manualcurrency))
                        @foreach($manualcurrency as $currency)
                          ...
                          <option {{ Session::get('currency')['mainid'] == $currency->currency->id ? "selected" : "" }} value="{{ $currency->currency->id }}">{{ $currency->currency->code }}
                           </option>
          ...
          <div class="container-fluid">
            <div class="row">
              <div class="col-6  col-md-2 col-sm-2 col-lg-2 logo-holder"> 
                <!-- ============ LOGO ========================================= -->
                <div  class="logo"> <a href="{{url('/')}}" title="{{$title}}"> <img height="50px" src="{{url('images/genral/'.$front_logo)}}" alt="logo"> </a> </div>
                <!-- /.logo --> 
                <!--=================== LOGO : END ================= --> </div>
              <!-- /.logo-holder -->
                          ...
                          <span class="input-group-btn">
                              <select id="searchDropMenu" class="" name="cat">
                                 <option value="all">{{ __('All Categories') }}</option>
                                  ...
                                  @foreach(App\Category::take(5)->orderBy('id','desc')->get(); as $cat)  
              ...
              <div class="col-lg-3 col-md-3 col-sm-3 col-0 animate-dropdown top-cart-row"> 
                ...
                <div class="dropdown dropdown-cart dropdown-cart-one">
                  <a href="#" class="dropdown-toggle lnk-cart" data-toggle="dropdown" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                             ...
                             @if(Session::get('currency')['position']== 'l' || Session::get('currency')['position'] == 'ls')
                                ...
                                <span class="value" id="total_cart">
                                 @if(Session::has('coupanapplied'))
                                   ...
                                   {{round((($total-session('coupanapplied')['discount'])*$conversion_rate),2)}}

                                @else 
                                    {{round(($total*$conversion_rate),2)}}
                                @endif
                                </span>
                        ...
                        <div class="cart-row">
                           <div class="row">
                            <div class="col-3 col-xs-3 col-sm-3 col-md-3">
                                <a href="{{url(url('details').'/'.$url)}}" title="{{$row->product->name}}"><img class="img-fluid pro-img2" src="{{url('/variantimages/'.$orivar->variantimages['image2'])}}" alt=""></a> 

                            </div>

                            <div class="col-7 col-xs-7 col-sm-7 col-md-7">
                             <h6 class="name"><a href="{{ url($url) }}" title="{{$row->product->name}}">{{$row->product->name}}&nbsp;&nbsp;(
                               ...
                               <div class="price">
                                ...
                                @if($row->ori_offer_price != 0 && $row->ori_offer_price != '')
                                    {{ round($row->ori_offer_price*$conversion_rate,2) }}
                                @else
                                    {{ round($row->ori_price*$conversion_rate,2) }}
                                @endif
                            ...
                            <div class="col-2 col-md-2 col-xs-2 col-sm-2">
                              <div class="action"> <a href="{{url('remove_table_cart/'.$row->variant_id)}}"><i class="fa fa-trash"></i></a> 
                            </div>
                            </div>
                          </div>
                           <hr/>  
                  ...
                  @if(!empty(Session::get('cart')))
                          ...
                          <div class="row">
                            <div class="col-3 col-xs-3 col-sm-3 col-md-3">
                                <a href="{{url($url)}}" title="{{$pro->name}}">
                                  <img class="pro-img2 img-fluid" src="{{url('variantimages/'.$orivar->variantimages['image2'])}}" alt="{{$pro->name}}">
                                </a> 

                            </div>

                            <div class="col-7 col-xs-7 col-sm-7 col-md-7">
                              <h6 class="name"><a href="{{ url($url) }}" title="{{$pro->name}}">{{$pro->name}}&nbsp;&nbsp;(
                               ...
                               <div class="price"><i class="{{session()->get('currency')['value']}}"></i>{{$row['varofferprice'] == 0 && $row['varofferprice']=='' ? $row['varprice']*$conversion_rate : $row['varofferprice']*$conversion_rate }}</div>
                            ...
                            <div class="col-2 col-md-2 col-xs-2 col-sm-2">
                              <div class="action"> <a href="{{url('remove_cart/'.$row['variantid'])}}"><i class="fa fa-trash"></i></a> 
                            </div>
                            </div>
                          </div>
                           <hr/>    
                      ...
                      <div class="clearfix cart-total cart-checkout">
                        <div class="float-right"> <span class="text">{{ __('staticwords.Subtotal') }} :</span><span class='price'>
                          @php
                        ...
                        @else
                            ...
                            @if($total != 0)
                            @if(!Session::has('coupanapplied'))
                              <i class="{{session()->get('currency')['value']}}"></i> {{ round($total*$conversion_rate,2) }}
                            @else
                              <i class="{{session()->get('currency')['value']}}"></i> {{ round(($total-session('coupanapplied')['discount'])*$conversion_rate,2) }}
                            @endif
                            @else
                             <i class="{{session()->get('currency')['value']}}"></i>0
                            @endif
                        ...
                        <div class="clearfix"></div>
                          ...
                          @endauth
                            ...
                            <button disabled="disabled" type="button" class="btn btn-upper btn-primary btn-block m-t-20">{{ __('staticwords.PROCCEDTOCHECKOUT') }}</button>
                            @else
                              <form action="{{url('checkout')}}" method="post" class="cart-checkout">
                                {{ csrf_field() }}
                                <input type="hidden" name="shipping" value="{{ $shipping }}">
                              <button type="submit" class="btn btn-upper btn-primary btn-block m-t-20">{{ __('staticwords.PROCCEDTOCHECKOUT') }}</button>
                            </form>
                            @endif
                            
                             @if(!empty($oot) && in_array(0, $oot) || $c!=0)
                            <a href="{{url('cart')}}" class="btn btn-upper btn-primary btn-block m-t-20" title="{{ __('Go To Cart') }}">{{ __('staticwords.GoToCart') }}</a> 
                            @endif
                      ...
                      @else
                       ...
                       <?php $token = md5(uniqid(rand(), true)); ?>
                        ...
                        <form action="{{ route('s.cart',$token) }}" method="POST" class="cart-checkout">
                          @csrf
                           <button type="submit" class="btn btn-upper btn-primary btn-block m-t-20">{{ __('staticwords.EmptyCart') }}</button>
                        </form> 
                    ...
                    <li class="notiheadergrey header">@if(auth()->user()->unreadnotifications->where('n_type','!=','order_v')->count())
                     ...
                     @endif 
                      ...
                      <ul class="menu notification-menu">
                         ...
                         @foreach(auth()->user()->unreadNotifications as $notification)
                          @if($notification->n_type != "order_v")
                          <li class="hey1 notiheaderlightgrey" id="{{ $notification->id }}" onclick="markread('{{ $notification->id }}')">
                          
                        
                            <small class="padding5P float-right"><i class="fa fa-clock-o" aria-hidden="true"></i> {{ date('jS M y',strtotime($notification->created_at)) }}</small>                     
                            <a class="color111 font-weight600"                       
                            @if($notification->n_type == "order")
                            @foreach(App\Order::where('order_id','=',$notification->url)->get() as $order)   --}}
                            href="{{ url('view/order/'.$notification->url) }}" 
                            @endforeach
            ...
            <div class="yamm navbar navbar-default" role="navigation">
              ...
              <div class="menu-new">
                  ...
                  <div class="smallscreen-bg-block">
                    <div class="smallscreen-bg-block-heading"><span id="welcomeHeading">{{ __('staticwords.Welcome') }} @auth {{ Auth::user()->name }} @endauth </span> </div>
                  </div>
                      ...
                      <div class="nav-outer"> 
                        ...
                        <ul class="nav navbar-nav">  
                                   ...
                                   <ul class="nav">
                                    ...
                                    @foreach(App\Category::where('status','1')->orderBy('position','ASC')->get(); as $item)
                                      ...
                                      <li class="dropdown menu-item" id="dropdown">
                                       ...
                                       <a aria-expanded="false" data-id="{{ $item->id }}" title="{{ $item->title }}" class="categoryrec cat-page">
                                        <i class="fa {{ $item->icon }}"></i>
                                        @if($price_login == 0 || Auth::check())

                                          <label class="cursor-pointer" onclick="categoryfilter('{{$item->id}}','','','{{ $startp }}','{{ $endp }}')">{{$item->title}}</label>
                                    ...
                                    <ul class="display-none" id="childOpen{{ $item->id }}">
                                        ...
                                        ?>
                                      @endif
                                        <li class="dropdown menu-item {{ $s->childcategory->count()>0 ? "" : "side-sub-list" }}">
                                        <a class="childrec" title="{{$s->title}}" data-id="{{ $s->id }}"><i class="fa {{ $s->icon }}"></i> 
                                        @if($price_login == 0 || Auth::check())

                                          <label class="cursor-pointer" onclick="categoryfilter('{{$item->id}}','{{ $s->id }}','','{{ $startp }}','{{ $endp }}')">{{$s->title}}</label>
                                                  ...
                                                  @foreach($s->childcategory->where('status','1')->sortBy('position') as $child)
                                                      ...
                                                      ?>
                                                        <li class="left-15 menu-item side-sub-list">
                                                          @if($price_login == 0 || Auth::check())
                                                            <a title="{{$child->title}}" class="cursor-pointer" onclick="categoryfilter('{{$item->id}}','{{ $s->id }}','{{ $child->id }}','{{ $startp }}','{{ $endp }}')"><i class="fa fa-angle-double-right" aria-hidden="true"></i> {{$child->title}}
                                                            </a>
                                                          @else
                                                            <a class="cursor-pointer" title="{{ $child['title'] }}"><i class="fa fa-angle-double-right" aria-hidden="true"></i> {{$child['title']}}</a>
                                                          @endif
                                                        </li>
                            ...
                            <!-- /.module-heading -->
                              ...
                              <div class="module-body">
                                <ul class='list-unstyled'>
                                  ...
                                  <li class="first"><a href="{{url('profile')}}" title="My Account">{{ __('staticwords.MyAccount') }}</a></li>
                          ...
                          <li>
                              ...
                              @foreach($footermenus->where('widget_postion','=','footer_wid_3')->where('status','1')->get() as $fm)        
                                <div class="module-body">
                                  <ul class='list-unstyled'>
                                    <li class="first">
                          ...
                          <li class="top-bar animate-dropdown">
                            <div class="header-top-inner">
                                <div class="cnt-account">
                                  ...
                                  @auth
                                   <li class="myaccount"><a href="{{url('profile')}}" title="My Account">{{ __('staticwords.MyAccount') }}</a></li>
                                   @inject('wish','App\Wishlist');
                                   @php
                                    $data = $wish->where('user_id',Auth::user()->id)->get();
                                    $count = [];
                                   ...
                                   @endphp
                                    ...
                                    <li class="check"><a data-toggle="modal" href="#feedone" title="Feedback">{{ __('staticwords.Feedback') }}</a></li>
                                    @endauth
                                    <li><a href="{{ route('hdesk') }}" title="Help Desk &amp; Support"><i class="fa fa-question-circle"></i>{{ __('staticwords.hpd') }}</a></li>
                        
                                    <!-- Feedback Modal -->
                                    <div data-backdrop="static" data-keyboard="false" class="z-index99 modal fade" id="feedone" tabindex="99" role="dialog" aria-labelledby="myModalLabel">
                                      <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                          <div class="feed-head modal-header">
                                          ...
                                          <div class="modal-body">
                                            ...
                                            <form action="{{ route('send.feedback') }}" method="POST">
                                              ...
                                              <div class="form-group">
                                                <label class="font-weight-bold" for="">{{ __('staticwords.Email') }}: <span class="required">*</span></label>
                                              <input required="" type="email" name="email" class="form-control" value="info@example.com">
                                              </div>
                                              <div class="form-group">
                                                <label class="font-weight-bold" for="">{{ __('staticwords.Message') }}: <span class="required">*</span></label>
                                                <textarea required="" name="msg" placeholder="{{ __('Tell us What You Like about us? or What should we do to more to improve our portal.') }}" cols="30" rows="10" class="form-control"></textarea>
                                              </div>
                                              <div class="rat">
                                              <label class="font-weight-bold">&nbsp;{{ __('staticwords.RateUs') }}: <span class="required">*</span></label>
                                              <ul id="starRating" data-stars="5" class="starrating-init">
                                              <li class="star"><i class="fa fa-star"></i></li><li class="star"><i class="fa fa-star"></i></li><li class="star"><i class="fa fa-star"></i></li><li class="star"><i class="fa fa-star"></i></li><li class="star"><i class="fa fa-star"></i></li></ul>
                                              <input type="hidden" id="" name="rate" value="1">
                                              </div>                     
      ...
      <div class="logo-slider-inner">
          ...
          <div id="brand-slider" class="owlCarousel brand-slider custom-carousel owl-theme">
           @foreach(App\Brand::where('show_image','1')->where('status','1')->orderBy('id', 'desc')->get() as $datas)
            <div class="item m-t-15"> <a href="#" class="image"> <img title="{{ $datas->name }}" width="100px" height="110px" data-echo="{{url('images/brands/'.$datas->image)}}" src="{{url('images/brands/'.$datas->image)}}" alt="{{ $datas->name }}"> </a> 
            </div>
          @endforeach
...
<footer id="footer" class="footer color-bg">
          ...
          <div class="module-body">
            <ul class="toggle-footer">
              ...
              <li class="media">
                ...
                <div class="media-body"> <span><a href="mailto:{{$genrals_settings->email}}" tiltle="E-Mail">{{$genrals_settings->email}}</a></span> </div>
              </li>
              @endif
            </ul>
          </div>
        ...
        <!-- /.col -->
          ...
          <div class="module-body module-small-screen">
            <ul class='list-unstyled'>
              ...
              @endif
              <li><a href="{{url('faq')}}" title="Faq">{{ __('staticwords.faqs') }}</a></li>
              <li class="last"><a href="{{ route('hdesk') }}" title="Help Center">{{ __('staticwords.HelpCenter') }}</a></li>
            </ul>
          </div>
          <!-- /.module-body --> 
        </div>
    ...
    <div class="container">
      <div class="row">
        ...
        <div class="col-xs-12 col-sm-4 col-lg-4 no-padding copyright">
          &copy; <?php echo date("Y"); ?> @if(isset($Copyright)) {{ $Copyright }}@endif 
        </div> 
           ...
           <div class="clearfix payment-methods">
            <ul>
              <li>
                ...
                <a title="PayPal" target="_blank" href="//paypal.com/"><img src="{{url('front/images/payments/1.png')}}" alt=""></a></li>
               @endif
              @if($Api_settings->paytm_enable=='1')
              <li><a title="PayTm" target="_blank" href="//paytm.com"><img src="{{url('images/images.jpg')}}" width="59px" height="39px" alt=""></a></li>
              @endif
              @if($Api_settings->razorpay=='1')
              <li><a title="RazorPay" target="_blank" href="//razorpay.com"><img src="{{url('images/rzpay.png')}}" width="59px" height="39px" alt=""></a></li>
              @endif
            @if($Api_settings->stripe_enable=='1')
              <li><a title="Stripe" target="_blank" href="//stripe.com"><img src="{{url('images/bluestripe.png')}}" width="70px" height="39px" alt=""></a></li>
            @endif
           @if($Api_settings->instamojo_enable=='1')
              <li><a title="InstaMojo" target="_blank" href="//instamojo.com"><img src="{{url('images/insta.png')}}" width="59px" height="39px" alt=""></a></li>
           @endif
            @if($Api_settings->payu_enable=='1')
               <li><a title="PayUMoney/Biz" target="_blank" href="//www.payumoney.com"><img src="{{url('images/payu.jpg')}}" width="59px" height="39px" alt=""></a></li>
            @endif           
            </ul>
          </div>
          <!-- /.payment-methods --> 
  ...
  <div class="copyright-bar header-nav-smallscreen">
   <button class="accordion no-padding social"><ul class="link">
         @foreach(App\Social::where('status','1')->get() as $social)
          <li class="{{$social->icon}} pull-left"><a target="_blank" rel="nofollow" href="{{$social->url}}"></a></li>         
           @endforeach
        </ul>
    ...
    <div class="panel max-height360">
      ...
      <div class="col-xs-12 col-sm-12 col-lg-12 no-padding btm-class">
        <div class="clearfix payment-methods">
          <ul>
            <li>
              @if($Api_settings->paypal_enable=='1')
              <a target="_blank" href="//paypal.com/"><img src="{{url('front/images/payments/1.png')}}" alt=""></a></li>
             @endif
            @if($Api_settings->paytm_enable=='1')
            <li><a target="_blank" href="//paytm.com"><img src="{{url('images/images.jpg')}}" width="59px" height="39px" alt=""></a></li>
            @endif
          @if($Api_settings->stripe_enable=='1')
            <li><a target="_blank" href="//stripe.com"><img src="{{url('images/bluestripe.png')}}" width="70px" height="39px" alt=""></a></li>
          @endif
         @if($Api_settings->instamojo_enable=='1')
            <li><a target="_blank" href="//instamojo.com"><img src="{{url('images/insta.png')}}" width="59px" height="39px" alt=""></a></li>
         @endif
          @if($Api_settings->payu_enable=='1')
             <li><a target="_blank" href="//www.payumoney.com"><img src="{{url('images/payu.jpg')}}" width="59px" height="39px" alt=""></a></li>
          @endif           
          </ul>
        </div>
        <!-- /.payment-methods --> 
      ...
      <div class="sidebar-widget newsletter outer-bottom-small">
          ...
          <div class="sidebar-widget-body outer-top-xs">
            <p>{{ __('newsletterword') }}</p>
            <form method="post" action="{{url('newsletter')}}">
                {{csrf_field()}}
              <div class="form-group">
                <label class="font-weight-bold" class="sr-only" for="">{{ __('staticwords.eaddress') }}</label>
                <input type="email" name="email" class="form-control" id="" placeholder="{{ __('staticwords.subscribeword') }}">
              </div>
...
<!-- Starrating JS -->
<script src="{{url('front/vendor/js/starrating.js')}}"></script>
<!-- jQuery Countdown JS -->
<script src="{{url('front/vendor/js/jquery.countdown.js')}}"></script>
<!-- Bootstrap Wizard JS --> 
<script src="{{url('front/vendor/js/jquery.bootstrap.wizard.js')}}"></script>
<!-- Tab JS -->
...
<script>var sendurl = "<?= route('ajaxsearch') ?>";</script>
<!-- Search JS -->
<script src="{{ url('js/search.js') }}"></script>

<script src="{{ url('js/htmlsession.js') }}"></script>
<script>
  var httpson = @if(isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on') 1 @else 0 @endif ;
  var sw = {!! json_encode( url('sw.js') ) !!};
  var rightclick = {!! json_encode($rightclick) !!};
  var inspect = {!! json_encode($inspect) !!};
</script>
<script src="{{url('js/frontmaster.js')}}"></script>
@yield('script')
</body>
</html>