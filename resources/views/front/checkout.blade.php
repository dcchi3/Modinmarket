  ...
  <div class="container-fluid">
    ...
    <div class="row">
        ...
        <div class="row no-gutters">
          <div class="col-1 col-md-1 col-lg-1 col-xl-1">
            <div class="checkout-page-img">
              <img align="left" class="pro-img img-responsive" src="{{url('variantimages/'.$orivar->variantimages['image2'])}}" alt="product_image">
            </div>
          </div>
          <div class="col-11 col-md-11 col-xl-11">
            <div class="checkout-page-dtl">
              ...
              <p class="pro-des pro-des-one"><b>{{__('staticwords.Price')}}:</b> 
                @if($row->product->vender_offer_price != '' && $row->product->vender_offer_price != NULL)
                        @php
                           $p=100;
                           $taxrate_db = $row->product->tax_r;
                           $vp = $p+$taxrate_db;
                           $tam = $row->product->vender_offer_price/$vp*$taxrate_db;

                 
                           $tam = sprintf("%.2f",$tam,2);
                        @endphp
                      @else
                        @php
                           $p=100;
                           $taxrate_db = $row->product->tax_r;
                           $vp = $p+$taxrate_db;
                           $tam = $row->product->vender_price/$vp*$taxrate_db;
                      ...
                      <p class="pro-des pro-des-one"> <b>{{__('staticwords.Tax')}} :</b>  
                        ...
                        <i class="fa {{ Session::get('currency')['value'] }}"></i>
                          @if($row->product->vender_offer_price != 0 && $row->product->vender_offer_price != NULL)
                            @php

                             $p=100;
                             $taxrate_db = $row->product->tax_r;
                             $vp = $p+$taxrate_db;
                             $tamount = $row->product->vender_offer_price/$vp*$taxrate_db;

                             echo $tamount = sprintf("%.2f",$tamount*$conversion_rate);
                            @endphp
                          @else
                            ...
                            @php
                             ...
                             $taxrate_db = $row->product->tax_r;
                             $vp = $p+$taxrate_db;
                             $tamount = $row->product->vender_price/$vp*$taxrate_db;

                               echo $tamount = sprintf("%.2f",$tamount*$conversion_rate);
                            @endphp
                          @endif
                         <b> [{{ $row->product->tax_r }}% ({{ $row->product->tax_name }})]</b>
      ...
      <div class="text-right col-12 offset-md-3 col-md-9 offset-xl-0 col-xl-3">
        <p><b>@if($row->product->vender_offer_price != 0 && $row->product->vender_offer_price != NULL)
                        @php
                                          ...
                                          if($store == $z)
                                          {
                                            ...
                                            if($taxRate->type == 'p')
                                            {
                                              ...
                                              $price = $row->ori_offer_price == 0 ? $row->ori_price : $row->ori_offer_price*$row->qty;
                                               ...
                                               $total_tax_amount += $after_tax_amount;
                                               App\Cart::where('id', $row->id)->update(array('tax_amount' => $after_tax_amount));
                                              ...
                                              $after_tax_amount =  $taxRate->rate;
                                               ...
                                               App\Cart::where('id', $row->id)
                                                      ->update(array('tax_amount' => $after_tax_amount));
                                                $after_tax_amount = sprintf("%.2f",$row->qty*$after_tax_amount);
                                  ...
                                  @else
                                    ...
                                    @if($row->product->vender_offer_price != '' && $row->product->vender_offer_price != NULL)
                                      @php
                                         ...
                                         echo $after_tax_amount = sprintf("%.2f",($actualtax*$conversion_rate));
                                         $total_tax_amount += $actualtax;
                                         App\Cart::where('id', $row->id)->update(array('tax_amount' => $actualtax));
                                      @endphp
                                    @else
                                      @php
                                         $p=100;
                 ...
                 @endforeach
                   ...
                   @endif
                          ...
                          <b>+ {{ sprintf("%.2f",$row->shipping) }}</b> <i class="{{ session()->get('currency')['value'] }}"></i>  <small class="text-muted"><b>( {{__('staticwords.Shipping')}} )</b></small>
                          <br>
                  ...
                  <br>
                      @php
                        ...
                        $listcheckOutCurrency  = App\CurrencyCheckout::where('currency','=',Session::get('currency')['id'])->first();
                          ...
                          $secure_amount = $secure_amount+$total_shipping+$total_tax_amount+$handlingcharge;
                ...
                <hr>
                 <form id="rpayform" action="{{ route('rpay') }}" method="POST" >
                          <script src="https://checkout.razorpay.com/v1/checkout.js"
                                  ...
                                  data-amount="{{ Crypt::decrypt($secure_amount)*100 }}"
                      ...
                      <div class="row">
                        ...
                        <div class="col-md-8 col-8">
                          <span class="" id="show-total">
                            ...
                            <i class="{{session()->get('currency')['value']}}"></i>{{ number_format((float)$total*$conversion_rate , 2, '.', '')}}
                          </span>
                        </div>
                      </div>
                    </div>
                    <div class="cart-sub-total totals-value" id="cart-total">
                      <div class="row">
                        ...
                        <div class="col-md-8 col-8">
                          ...
                          <i class="{{session()->get('currency')['value']}}"></i>{{sprintf("%.2f",$total_tax_amount*$conversion_rate,2)}}
                     ...
                     @endif
                      ...
                      <div class="row">
                        ...
                        <div class="col-md-8 col-5">
                          <span class="" id="show-total">
                              <i class="{{session()->get('currency')['value']}}"></i>{{  sprintf("%.2f",$handlingcharge*$conversion_rate,2) }}*
                          </span>
                        </div>
                      </div>
                    </div>
                        ...
                        <div class="col-lg-8 col-8">
                          ...
                          <span class="" id="gtotal">
                            @php
                              ...
                              $secure_pay = sprintf("%.2f",$secure_pay*$conversion_rate,2);
                            @endphp
                          <i class="{{session()->get('currency')['value']}}"></i>
                          <span id="grandtotal">
                         ...
                         <?php session()->put('payamount',sprintf("%.2f",Crypt::decrypt($secure_amount),2)); ?>
                        </div>
                      </div>
                    </div>
