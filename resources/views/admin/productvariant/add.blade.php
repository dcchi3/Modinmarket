...
@section("body")
	...
	<div class="box-header with-border">
		<div class="box-title">
			<a title="Go back" href="{{ url('admin/products') }}" class="btn btn-sm btn-default"><i class="fa fa-reply" aria-hidden="true"></i></a> {{$findpro->name}}'s Variant
		</div>
	</div>
		    	...
		    	<div class="box box-danger">
		    		<div class="box-header with-border">
		    			<div class="box-title">
			  	  		...
			  	  		<a @if(env('DEMO_LOCK') == 0) data-toggle="modal" href="#cmModal" @else disabled title="This action is disabled in demo !" @endif class="margin-right-8 pull-right btn btn-md btn-primary">
							...
							<tbody>
								...
								<tr>
									...
									<td>
										...
										@if($nameofval->unit_value != null && strcasecmp($nameofval->unit_value, $nameofval->values) != 0)
									...
									<td>
										...
										<a @if(env('DEMO_LOCK') == 0) data-target="#cmdel{{ $commonvariant->id }}" @else disabled title="This action is disabled in demo !" @endif class="btn btn-sm btn-danger" data-toggle="modal">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								...
								<button @if(env('DEMO_LOCK') == 0) data-toggle="modal" href="#{{ $opt->id }}var" @else disabled="" @endif class="btn btn-sm btn-danger">	<i class="fa fa-trash-o"></i>
								</button>
							</td>
		    ...
		    <div role="tabpanel" class="tab-pane" id="profile">
				...
				<div class="box box-danger">
					<div class="box-header with-border">
						...
						<a class="btn btn-md btn-primary pull-right" href="{{ route('manage.stock',$findpro->id) }}">+ ADD
						</a>
					</div>
    			...
    			@foreach($findpro->subvariants as $key=> $sub)
					<tr>
						...
						<td class="v-middle custom-radios">
							...
							<input class="cmn" id="{{ $sub->id }}" onclick="setdef('{{ $sub->id }}','{{ $findpro->id }}')" type="radio" {{ $sub->def==1 ? "checked" : "" }}>


							<label for="{{ $sub->id }}">
								<span>
						        <img class="align-unset" src="https://s3-us-west-2.amazonaws.com/s.cdpn.io/242518/check-icn.svg" alt="Checked Icon" />
						      </span>
							</label>

							<style>
								.custom-radios input[type="radio"][id="{{ $sub->id }}"] + label span {
								  background-color: #157ed2;
								}

								.custom-radios input[type="radio"]:not(:checked)[id="{{ $sub->id }}"] + label span {
						...
						<td class="v-middle">
							<a title="Edit variant" href="{{ route('edit.var',$sub->id) }}" class="btn btn-md btn-sm btn-primary">
								<i class="fa fa-pencil"></i>
							</a>

							<a @if(env('DEMO_LOCK') == 0) data-toggle="modal" href="#deletevar{{ $sub->id }}" @else title="This action is disabled in demo !" disabled @endif class="btn btn-sm btn-danger">
								<i class="fa fa-trash-o"></i>
							</a>
						</td>
	      ...
	      <div class="modal-body">
	       		<form action="{{ route('common.update',$commonvariant->id) }}" method="POST">
	       		 ...
	       		 @foreach($commonvariant->attribute->provalues as $attrval)
	       		 	<label>
	       		 		<input {{ $commonvariant->cm_attr_val == $attrval->id ? "checked" : "" }} type="radio" name="cm_attr_val" value="{{ $attrval->id }}">
