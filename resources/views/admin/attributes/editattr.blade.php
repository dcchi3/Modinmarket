							...
							<div class="box-body">
								...
								@if(isset($old_values) && count($old_values) > 0)
					      			@foreach($old_values as $old_value)
					      				<label>
										<input checked type="checkbox" name="cats_id[]" value="{{ $old_value }}">
										@php
										  $getcatname = App\Category::where('id',$old_value)->first()->title;
										@endphp
										{{$getcatname}}
										</label>
					      			@endforeach
					      		...
					      		@if(isset($diff_values))
									@foreach($diff_values as $orivalue)
										<label>
											...
											@php
										  $getcatname = App\Category::where('id',$orivalue)->first()->title;
										@endphp
										{{ $getcatname }}
										</label>
									@endforeach
