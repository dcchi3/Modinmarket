            ...
            <div id="maincat" class="maincat form-group {{ $menu->show_cat_in_dropdown == 1 ? '' : 'display-none' }}">
                ...
                <div id="test{{$item->id}}" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
                  @php
                   ...
                   $data = $item->subcategory->where('status','1');
                        ...
                        $single_data = $data[0];
                          @endphp
                             <div class="col-md-12 sub-cat-sel">
                              <label><input @if($menu->show_cat_in_dropdown == 1 && isset($childcats)) @foreach($childcats as $c){{ $c == $single_data['id'] ? "checked" : "" }} @endforeach @endif type="checkbox" name="child_cat[]" class="required_one sub_categories sub_categories_{{$item->id}}" parents_id = "{{$item->id}}" value="{{$single_data['id']}}"> {{$single_data['title']}}</label>
                            </div>
                        @php
                      }else{
                        for ($j; $j < $t; $j += 2) {
                          ...
                          <div class="col-md-6 sub-cat-sel">
                            <label><input @if($menu->show_cat_in_dropdown == 1 && isset($childcats)) @foreach($childcats as $c){{ $c == $left_row['id'] ? "checked" : "" }} @endforeach @endif type="checkbox" name="child_cat[]" class="required_one sub_categories sub_categories_{{$item->id}}" parents_id = "{{$item->id}}" value="{{$left_row['id']}}"> {{$left_row['title']}}</label>
                          </div>
                            @php
                          $i += 2;
                          ...
                          @endphp
                          <div class="col-md-6 sub-cat-sel">
                            <label><input @if($menu->show_cat_in_dropdown == 1 && isset($childcats)) @foreach($childcats as $c){{ $c == $right_row['id'] ? "checked" : "" }} @endforeach @endif type="checkbox" name="child_cat[]" class="required_one sub_categories sub_categories_{{$item->id}}" parents_id = "{{$item->id}}" value="{{$right_row['id']}}"> {{$right_row['title']}}</label></div>
